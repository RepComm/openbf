...
class NodeTERR {
    ...
    parse (filepath, callback) {
        ...
        try {
            ...
            this.error = "Error while trying to read " + filepath + " to parse as TER terrain file";
            callback(this.error);
        }
        if (buf.toString("utf8", 0, 4) !== "TERR") {
            this.error = filepath + " may not be a terrain file, it doesn't even have a good header!";
            callback(this.error);
        }
        ...
        callback(error, terrainData);
    }
}

module.exports = NodeTERR;